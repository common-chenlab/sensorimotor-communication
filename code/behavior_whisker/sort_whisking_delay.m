function [X, Y, W, Z] = sort_whisking_delay(summary, whisker_dat, trials, min_frames, sort_type, task, angle, samplingrate)
...
for i = 1:length(trials)
    ...
    if trials(i).delay == 2000
        %     calc_min = [calc_min; trials(i).decision_time-trials(i).direction_2_time+trials(i).delay_time];
        calc_min = [calc_min; trials(i).recording_time];
    end
    %     calc_min = [calc_min; trials(i).report_time-trials(i).direction_2_time+trials(i).delay_time];
end
% min_frames = round(prctile(calc_min,10)/1000*Ca.sampling_rate)-min_offset;
...
for j = 2:size(summary.table,1)
    ...
    if isempty(CCD) == 0
        ...
        if isempty(whisking) == 0
            ...
            %         try
            %                 whisking = [whisking(start:d1_end), whisking(d2_time:end)];
            ...
            if length(whisking) >= min_frames
                ...
                %                 j
                %                 if trials(trial_id).task == task
                %                     if task == 1
                %                     if findstr('CW 40', trials(trial_id).direction_1) == 1
                %                         if findstr('CW 40', trials(trial_id).direction_2) == 1
                %                             CW_CW = [CW_CW; whisking];
                %                         else
                %                             CW_CCW = [CW_CCW; whisking];
                %                         end
                %                     elseif findstr('CCW 40', trials(trial_id).direction_1) == 1
                %                         if findstr('CW 40', trials(trial_id).direction_2) == 1
                %                             CCW_CW = [CCW_CW; whisking];
                %                         else
                %                             CCW_CCW = [CCW_CCW; whisking];
                %                         end
                %                     end
                ...
                if findstr('Miss', trials(trial_id).decision) > 0
                    Miss = [Miss; whisking];
                end
                if findstr('CR', trials(trial_id).decision) > 0
                    CR = [CR; whisking];
                end
                if findstr('FA', trials(trial_id).decision) > 0
                    FA = [FA; whisking];
                end
            end
            %             end
