function rrr = compile_correlation_binned(animal, session, all)
    ...
    for k = 1:length(ix)        
        for j = 1:3
            ...
            for i = 1:2 %size(temp,1)                
                if i == 1
                    r = corrcoef(temp(sampletime,ix{k},j),temp2(sampletime,ix{k},j));
                else
                    r = corrcoef(temp(testtime,ix{k},j),temp2(testtime,ix{k},j));
                end
                result(i) = r(1,2);
            end
            rrr(j,:,m,k) = result;
