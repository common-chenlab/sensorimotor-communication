...
% keyboard
%         temp = bigread2(Y, 1, length(tiffInfo));

        info.filename = Y;
        temp = UCLA_ReadTiff(info);
        
        Y = double(reshape(temp,[],length(tiffInfo)));
    end
end
if memmaped
%     sizY = size(Y,'motion_corrected');
    sizY = size(Y,'Y');
    d = prod(sizY(1:end-1));
    T = sizY(end);
else
...
% ??? we use contraint foopsi (default), restimate_g seems to flag for estimation of time-decays, but it's not used later ...
...
if options.temporal_parallel
    ...
    perm = 1:K+size(b,2); randperm(K+size(b,2));
        for jj = 1:K
            ...
            %if mod(jj,10) == 0, fprintf('%i out of total %i temporal components updated \n',jj,K); end
        end
        if norm(Cin - C,'fro')/norm(C,'fro') <= 1e-3
            % stop if the overall temporal component does not change by much
            break;
