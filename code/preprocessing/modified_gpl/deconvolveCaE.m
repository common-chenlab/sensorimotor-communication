function [c, s] = deconvolveCaE(y, varargin)
...
switch lower(options.method)
    case 'foopsi'  %% use FOOPSI
        ...
        elseif strcmpi(options.type, 'exp2')   % difference of two exponential functions
            d = options.pars(2);
            r = options.pars(1);
            %   options.pars = (exp(log(d)*(1:win)) - exp(log(r)*(1:win))) / (d-r);
            %  options.pars=  exp( ((1:win)./1000)./r ) - exp( ((1:win)./1000)./d )  ;
            %options.pars= exp( ((1:win)./1000)./d )- exp( ((1:win)./1000)./r )   ;
            SR=sampling_rate./1000;tt=1:win;tt=tt-0.9;r_con=r;d_con=d;
            rstm= exp((tt*SR)*-(1000./r_con) ) ;
            dtm= exp((tt*SR)*-(1000./d_con) ) ;
            options.pars=dtm-rstm;
    ...
    case 'thresholded'  %% Use hard-shrinkage method
        ...
        elseif strcmpi(options.type, 'exp2')   % difference of two exponential functions
            ...
            %options.pars=  exp( ((1:win)./1000)./r ) - exp( ((1:win)./1000)./d )  ;
            %options.pars= exp( ((1:win)./1000)./d )- exp( ((1:win)./1000)./r )   ;
            SR=1./sampling_rate;tt=(1:win);
            rstm=exp((tt*SR)*-(1000./r));
            dtm=exp((tt*SR)*-(1000./d));
            options.pars=(dtm-rstm);
            %  figure,plot( dtm-rstm )
            % convolution kernel
            [c, s] = onnls(y-options.b, options.pars, options.lambda, ...
                options.shift, options.window, [], [], [], options.smin);
        elseif strcmpi(options.type, 'kernel') % convolution kernel itself
...
options.window = round(2000/30);
options.shift = round(300/30);
options.smin = 0;
options.maxIter = 10;
...
while k<=nargin
    ...
    switch lower(varargin{k})
        ...
        case 'thresh_factor'
            ...
            k = k+2;
        case 'sampling_rate'
            options.sampling_rate = varargin{k+1};
            k = k+2;
        otherwise
            k = k+1;
    end
